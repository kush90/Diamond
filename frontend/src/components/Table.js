...
const Table = ({ title, header, data, editData, deleteData, changePassword, productData, confirmDeal, viewDetail, confirmDeliver, confirmPayment, viewCommission }) => {
                    ...
                    data.length > 0 ? data.map((value, index) => {
                        return (
                            <tr key={index} >
                                ...
                                {(title === 'product' || title === 'category' || title === 'user') && <td className="text-truncate text-primary pointer" style={{ maxWidth: 116 }}>{value.name}</td>}
                                ...
                                {(title === 'deal' || title === 'order') && <td>{formatDateToLocaleString(value.createdAt)}</td>}
